...
export class Game {
  ...
  removeLine(lineId) {
    ...
    if (idx > -1) {
      ...
      const cost =
        GLOBAL_CONFIG.line_base_cost * Math.pow(GLOBAL_CONFIG.line_cost_growth, count - 1)
  ...
  getLineCost() {
    // base * growth^(n)
    return (
      GLOBAL_CONFIG.line_base_cost * Math.pow(GLOBAL_CONFIG.line_cost_growth, this.lines.length)
    )
  }
