...
async function loadTranslation(lang) {
  ...
  if (lang === 'ko') {
    translations.ko = (await import('./translations/ko.js')).default
  } else if (lang === 'en') {
    translations.en = (await import('./translations/en.js')).default
  }
...
export function t(key, params = {}, fallback = null) {
  ...
  if (text === undefined) {
    ...
    if (__IS_DEV__ && !__warnedKeys.has(key)) {
      console.warn(`[i18n] Missing translation key: ${key} (currentLang=${currentLang})`)
...
// 번역 번들을 로드하기 위한 함수 (async)
export async function ensureTranslationLoaded(lang) {
  const resolved = resolveLang(lang) || 'ko'
  if (translations[resolved]) return translations[resolved]
  return await loadTranslation(resolved)
}
...
export function setLang(lang) {
  ...
  // 번역 번들 백그라운드 로드 (비동기, await 하지 않음)
  ensureTranslationLoaded(resolved)
