...
function toBigIntSafe(value, fieldName = 'unknown') {
  ...
  if (__IS_DEV__) {
    ...
    if (wasDecimal || wasNegative) {
      ...
      console.debug(
        `[LB] ${fieldName} converted from ${num} to ${safeValue} (${reasons.join(', ')})`
      )
    }
  }
...
export async function updateLeaderboard(
  ...
  try {
    ...
    if (!forceImmediate && __lastUpdateSuccess && now - __lastUpdateTime < throttleMs) {
      if (__IS_DEV__) {
        console.debug(
          `[LB] Throttled: ${Math.round((throttleMs - (now - __lastUpdateTime)) / 1000)}s remaining`
        )
      }
...
export async function getMyRank(nickname, sortBy = 'assets') {
  ...
  try {
    ...
    if (error) {
      ...
      const errType = status === 401 || status === 403 ? 'forbidden' : 'generic'
      return {
        success: false,
        data: null,
        error: error.message,
