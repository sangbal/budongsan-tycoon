...
export async function initAuthUI(opts) {
  ...
  const { initAuthFromUrl, getUser, isAuthEnabled, onAuthStateChange, signInWithOAuth, signOut } =
...
async function handleDeleteAccount(scope, toast, doLogout, deleteAccount) {
  ...
  try {
    const keysToDelete = ['clicksurvivor-auth', 'seoulTycoonSaveV1']
